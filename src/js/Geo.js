...
export default class Geo {
  ...
  askCoords() {
    ...
    this.modal
      .querySelector(".timeline_geo_close")
      .addEventListener("click", () => this.modal.remove());
    this.modal
      .querySelector("form.timeline_geo_form")
      .addEventListener("submit", this.checkValidity);
  }

  checkValidity(event) {
    event.preventDefault();
    const geoInput = this.geoInputFormat(
      this.modal.querySelector(".timeline_geo_input").value,
    );
    if (geoInput.error) {
      this.geoShowError(
        this.modal.querySelector(".timeline_geo_input"),
        "Введите значение в формате 00.00, 00.00",
      );
      return;
    }
